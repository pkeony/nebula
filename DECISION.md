...
## D15. 세션 관리 — In-memory Snapshot

**결정:** 세션을 메모리 Map으로 관리, AgentStreamState 전체를 스냅샷 저장

**선택지:**
| 옵션 | 장점 | 단점 |
|------|------|------|
| **In-memory Snapshot** | 즉시 구현, 외부 의존 없음 | 새로고침 시 유실 |
| LocalStorage | 영속성, 오프라인 | 용량 제한 5MB, 직렬화 비용 |
| IndexedDB | 대용량, 비동기 | API 복잡 |

**근거:**
- MVP 단계에서 외부 저장소 의존 없이 빠른 검증
- 세션 전환 시 상태 완벽 복구 (메시지 + events + 스트리밍 상태)
- 인터페이스가 Map이므로 Redis/IndexedDB로 교체 시 래퍼만 변경
- D12(대화 저장소 In-memory Map)와 동일 철학: "의도적 단순화 → 점진적 확장"

---

## D16. ProcessTimeline 우측 패널 분리

**결정:** ProcessTimeline을 독립 우측 패널(`w-96`)로 분리

**선택지:**
| 옵션 | 장점 | 단점 |
|------|------|------|
| Flow 내부 오버레이 | 구현 단순 | Flow 조작 방해 |
| 하단 콘솔 패턴 | 익숙한 UX (IDE 패턴) | 세로 공간 분할 |
| **우측 독립 패널** | 병렬 표시, 독립 스크롤 | 모바일 공간 부족 |

**근거:**
- Flow 시각화와 이벤트 타임라인을 동시에 볼 수 있음 (정보 밀도↑)
- 이벤트 없을 때 자동 숨김 (공간 낭비 없음)
- 데스크톱 우선 UX — 포트폴리오 데모가 주 사용 시나리오
- 모바일 대응은 탭 전환으로 진화 가능

---

## D17. 포트폴리오형 랜딩페이지 분리

**결정:** `/` → 랜딩페이지, `/chat` → 채팅 UI 분리

**선택지:**
| 옵션 | 장점 | 단점 |
|------|------|------|
| 단일 페이지 (채팅만) | 유지보수 단순 | 프로젝트 설명 없음 |
| **랜딩 + 채팅 분리** | 포트폴리오 동선 최적화 | 두 페이지 유지보수 |
| 외부 랜딩 (Notion 등) | 빠른 구축 | 기술력 미어필 |

**근거:**
- 채용 담당자 기대 흐름: 프로젝트 소개 → 기술 스택 확인 → 데모 체험
- 랜딩페이지 자체가 프론트엔드 역량 증명 (애니메이션, 반응형, 디자인 시스템)
- SEO 최적화 가능 (각 페이지 별도 메타 태그)

---

## D18. run_command 보안 — 화이트리스트 + 차단 패턴 이중 레이어

**결정:** 화이트리스트(허용 접두사) + 차단 패턴(정규식) 이중 방어

**선택지:**
| 옵션 | 장점 | 단점 |
|------|------|------|
| 화이트리스트만 | 단순, 안전 | 새 명령어 추가 시 수정 필요 |
| 차단 패턴만 | 유연 | 우회 가능성 (새 위험 명령 미차단) |
| **이중 레이어** | 방어 깊이 최대 | 관리 복잡도 소폭 증가 |

**근거:**
- 차단 패턴만으로는 새로운 위험 명령어 등장 시 우회 가능
- 화이트리스트만으로는 정상 명령어의 위험 조합 (`ls; rm -rf /`) 미차단
- 두 레이어를 결합하면: 1) 차단 패턴 우선 체크 → 2) 화이트리스트 통과만 실행
- 면접에서 "Defense in Depth" 보안 원칙 설명 가능
...
| 시도 | 결과 | 교훈 |
|------|------|------|
| Flow 엣지 bezier + Loose 모드 | 엣지가 노드를 관통하거나 겹침 | smoothstep/직각 꺾임이 정보 흐름 표현에 더 적합 |
| Flow 좌→우 wrap 레이아웃 | 도구 호출 쌍이 시각적으로 연결되지 않음 | 세로 흐름 + 도구 페어링(좌/우)이 Agent 실행 구조를 더 잘 표현 |
| React Flow fitView() 자동 추적 | 전체 그래프를 보여주려다 최신 노드가 작아짐 | setCenter()로 최신 노드만 추적하는 것이 실시간 모니터링에 적합 |
