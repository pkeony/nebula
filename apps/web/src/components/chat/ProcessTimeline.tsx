...
function buildSteps(events: AgentEvent[], isStreaming: boolean): TimelineStep[] {
  ...
  for (const event of events) {
    if (event.type === 'thinking') {
      ...
      steps.push({
        ...
        status: hasResult ? 'done' : 'active',
      });
    } else if (event.type === 'tool_result' && event.isError) {
      steps.push({
        type: 'tool_result',
        label: `${formatToolName(event.tool)} Error`,
        detail: event.result.slice(0, 80),
        icon: 'error',
        status: 'error',
      });
    }
...
function StatusBadge({ status }: { status: TimelineStep['status'] }) {
  if (status === 'done') {
    return (
...
export function ProcessTimeline({ events, isStreaming }: ProcessTimelineProps) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex justify-between items-end px-8 pt-8 pb-4 flex-shrink-0">
        ...
        {isStreaming && toolCount > 0 && (
          <span className="text-[10px] uppercase tracking-widest font-bold text-[var(--color-primary)] px-2.5 py-1 bg-[var(--color-primary-container)] rounded-full">
            Step {doneCount} of {toolCount}
          </span>
        )}
        {!isStreaming && hasContent && (
          <span className="text-[10px] uppercase tracking-widest font-bold text-[var(--color-primary)] px-2.5 py-1 bg-[var(--color-primary-container)] rounded-full">
            {toolCount} steps done
          </span>
        )}
      </div>
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={i} className="flex items-start gap-5 relative group">
              ...
              <div
                className={`
                  w-12 h-12 rounded-2xl flex items-center justify-center z-10 flex-shrink-0 transition-all
                  ${isActive
      ...
      {/* 하단 — Contextual Insights (Helpful Aura) */}
      {!isStreaming && hasContent && (
        <div className="pt-8">
          <div className="glass-panel p-6 rounded-2xl relative overflow-hidden" style={{ border: '1px solid rgba(237,218,200,0.3)' }}>
            ...
            <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed">
              {toolCount} tool{toolCount !== 1 ? 's' : ''} executed, {doneCount} completed successfully.
            </p>
          </div>
        </div>
      )}
